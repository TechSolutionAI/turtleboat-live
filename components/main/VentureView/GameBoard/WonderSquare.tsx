...
const WonderSquare = ({ size, data, content, style }: WonderSquareProps) => {
  ...
  const handleModuleClicked = () => {
    if ((content == null || content == undefined ) && data._id != '' && data.isCheck && !data.isLock) {
      router.push(`/dashboard/ventures/module/${data.ventureId}-${data._id}`);
    }
  }
  ...
  return (
    <div
      className="wonder-square-container relative cursor-pointer"
      style={{
        width: `${size}px`,
        height: `${size}px`,
        pointerEvents: "none",
            ...
            data._id != '' && data.isCheck ?
              data.isLock ?
                <div className="flex justify-center">
                  {/* <Image src={LockClosed} className="w-[30px] tooltip" alt="" /> */}
                  <LockClosed className="w-[30px] tooltip" alt="" />
                  </div>
              :
              // <div className="flex justify-center" ><Image src={LockOpened} className="w-[30px]" alt="" /></div>
