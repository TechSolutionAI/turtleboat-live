...
const ComicStripEditor = ({
  ...
  const [visibleDrawingCursor, setVisibleDrawingCursor] =
    ...
    if (layerRef.current && isEditable) {
      ...
      if (groupRef.current && dataURL.length > 6) {
        ...
        updatePanel({
          thumbnail: dataURL,
          thumbPubId:
            panelData != undefined ? panelData[selectedPanel]?.thumbPubId : "",
  ...
  useEffect(() => {
    ...
    // const pencilImg = new window.Image();
    // pencilImg.src = '/static/images/pencil.svg';
    // pencilImg.onload = () => {
    //     setPencilImage(pencilImg);
    // };

    // const eraserImg = new window.Image();
    // eraserImg.src = '/static/images/eraser.svg';
    // eraserImg.onload = () => {
    //     setEraserImage(eraserImg);
    // };
  ...
  const drawElements = () => {
    if (groupRef.current) {
      ...
      if (
        ...
        panelData[selectedPanel] != undefined
      ) {
        setIsLoading(true);
      ...
      if (tool != "drawing") {
        ...
        if (stageRef.current) {
          ...
          textarea.style.width =
            nodeItem.width() - nodeItem.padding() * 2 + "px";
  ...
  return (
    <div>
      <div
        ref={stageRef}
        className={`relative bg-comic_bg border-[#424242] border-2 comic-stage`}
      >
        {isLoading ? (
          <div
      ...
      {isEditable && (
        <div className="font-Inter flex items-center justify-center">
          <a
            className="cursor-pointer flex items-center gap-x-2 text-[#4F4F4F]"
            onClick={() => {
              showTipsModal(true);
            }}
          >
            <HelpIcon /> Tips For Creating The {type} Comic Strip
          </a>
        </div>
      )}
