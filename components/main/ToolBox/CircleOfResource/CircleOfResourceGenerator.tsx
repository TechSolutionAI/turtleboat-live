...
const CircleOfResourceGenerator = () => {
  ...
  const getVenture = async () => {
    if (ventureId != "") {
      ...
      if (!response.ok) {
        ...
        if (
          venture.circleOfResource != undefined &&
          venture.circleOfResource != null
        ) {
          setCirleOfResourcesTableData(venture.circleOfResource);
          getCircleOfDistances(venture.circleOfResource);
        }
        setVenture(venture);
        setIsLoading(false);
      }
    ...
    if (session != undefined) {
      ...
      const response = await fetch("/api/circleofresource", {
        ...
        body: JSON.stringify({
          ...
          data: cirleOfResourcesTableData,
        }),
      });
      ...
      if (!response.ok) {
        ...
        Swal.fire({
          ...
          text: `Circle of Resources were saved successfully!`,
        })
          .then(() => {})
          .catch((err) => console.log(err));
      }
  ...
  useEffect(() => {
    document.title = "ToolBox - Circle of Resource Generator - Turtle Boat";
    getVenture();
  }, []);
      ...
      {isLoading ? (
        ...
        <>
          <div className="flex justify-between items-center sticky top-0 bg-white z-10 pt-2 pb-4">
            <div className="flex items-center text-[20px] font-Inter font-bold lg:flex p-2">
