...
server.get('/server-api/saveninetyvideo', (req, res) => {
    res.status(200).json({ success: false, err: SERVER_ERR_MSG });
})

server.post('/server-api/saveninetyvideo', async (req, res) => {
        ...
        form.parse(req, async (err, fields, files) => {
            ...
            try {
                ...
                fileObject.filepath,
                {
                    ...
                    //     format: "mp4",
                    //     transformation: [
                    //     { width: 320, height: 240, crop: "pad" },
                    //     { audio_codec: "aac", bit_rate: "500k" },
                    //     { quality: 60, fps: 30 },
                    //     ],
                    // },
                    // {
...
app.prepare().then(() => {
    server.listen(3000, (err) => {
        if (err) throw err;
        console.log(`> Ready on ${process.env.HOME_URL}`);
    });
});