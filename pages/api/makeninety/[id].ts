    ...
    try {
        ...
        if (type == 1) {
            ...
            if (!result.matchedCount) {
                ...
                const video = await db.collection("ninetyvideos").findOne({ _id: videoId });
                if (video.author._id.toString() !== userId.toString()) {
                    ...
                    pusher.trigger(`user-${video.author._id.toString()}`, 'comment', notificationForComment);
                }
...
async function updateVideoStatus(req: NextApiRequest, res: NextApiResponse) {
    ...
    try {
        ...
        var message = status == 'public' ? 'Your 90 second sound bite has been approved and posted onto CORE tab.  Videos will be deleted after 90 days.' : 'Your 90 second sound bite was not approved.  The most likely reason is that your video is over 95 seconds long.';
        ...
        try {
            await sendgrid.send({
                ...
                subject: status == 'public' ? 'Your 90 second sound bite has been approved' : 'Your 90 second sound bite was not approved.',
