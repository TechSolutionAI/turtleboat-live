...
async function saveComment(req: NextApiRequest, res: NextApiResponse) {
    ...
    form.parse(req, async (err, fields, files) => {
        ...
        await db.collection("ventures").updateOne(
            {
                _id: ventureId,
                "stakeholderScenario": { $exists: false },
            },
            {
                $set: {
                    "stakeholderScenario.comments": [],
                },
            }
        );
        ...
        if (!result.matchedCount) {
            ...
            if (
                ...
                const notificationForComment = {
                    ...
                    link: `/dashboard/toolbox/characterbrainstorm/${ventureId}`,
                    isRead: false,
                    isFlag: false,
            ...
            // Get Token Action for "Post/Comment within a Brainstorm": no is 19
