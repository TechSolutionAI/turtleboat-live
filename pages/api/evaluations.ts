    ...
    const result = await db.collection("ventures").updateOne(
      {
        _id: ventureId,
        "course.modules.module._id": moduleId.toString(),
        "course.modules.evaluations._id": userId.toString(),
      },
      {
        $set: {
          "course.modules.$.evaluations.$[evaluation].value": value,
          "course.modules.$.evaluations.$[evaluation].lastUpdated": new Date(),
        }
      },
      {
        arrayFilters: [
          { "evaluation._id": userId.toString() }
        ]
      }
    );
